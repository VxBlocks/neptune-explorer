...
use neptune_explorer::rpc::mempool::{mempool_overview, pow_puzzle_internal_key};
...
pub fn setup_routes(app_state: AppState) -> Router {
    Router::new()
        ...
        .route("/rpc/mempool/:start_index/:number", get(mempool_overview))
        .route("/rpc/pow_puzzle", get(pow_puzzle_internal_key))
        // -- Dynamic HTML pages --
        .route("/", get(root))
